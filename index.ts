...
let spinHistoryArray: SpinRecord[] = [];
...
function statsBtnHandler() {
  ...
  if (amount > 0) {
    ...
    statsResults.innerText = `No spins`;
  }
}
...
function getAmount(colour, bodyPart): number {
  ...
  for (let i = 0; i < spinHistoryArray.length; i++) {
    let Temp = spinHistoryArray[i];
    if (Temp.colour == colour && Temp.bodyPart == bodyPart) Amt++;
  }
  return Amt;
}
...
function getLastSpun(colour, bodyPart): number {
  for (let i = spinHistoryArray.length - 1; i >= 0; i--) {
    let Temp = spinHistoryArray[i];
    if (Temp.colour == colour && Temp.bodyPart == bodyPart) return Temp.num;
  }
  return -1;
}
